<?php
function open_msupe_preprocess_html(&$vars) {
...
function open_msupe_js_alter(&$javascript) {
  // Update jquery version for non-administration pages
  if (arg(0) != 'admin' && arg(0) != 'panels' && arg(0) != 'ctools') {
    $jquery_file = drupal_get_path('theme', 'open_msupe') . '/js/jquery-1.9.1.min.js';
    $jquery_version = '1.9.1';
    $migrate_file = drupal_get_path('theme', 'open_msupe') . '/js/jquery-migrate-1.1.1.min.js';
...
function open_msupe_preprocess_page(&$vars) { 
...
function open_msupe_preprocess_block(&$vars) {
...
function open_msupe_region_has_block($region) {
...
function open_msupe_get_span($block_count, $block_id, $count_sidebars) {
 ...
 */
function open_msupe_status_messages($variables) {
...
function open_msupe_form_alter(&$form, &$form_state, $form_id) {
  if ($form_id == 'search_block_form') {
    ...
    $form['actions']['submit']['#src'] = drupal_get_path('theme', 'open_msupe') . '/images/searchbutton.png';
  }
}

/* Returns HTML for primary and secondary local tasks. */
function open_msupe_menu_local_tasks(&$vars) {
  $output = '';
...
function open_msupe_menu_local_task($vars) {
...
function open_msupe_menu_tree(&$vars) {
...
function open_msupe_menu_link(array $vars) {

  $element = $vars['element'];

  if (open_msupe_is_in_nav_menu($element)) {
    $sub_menu = '';
...
function open_msupe_item_list($variables) {
...
function open_msupe_is_in_nav_menu($element) {
  ...
  if (!isset($nav_theming_functions)) {
    ...
    $nav_theming_functions = array_map('open_msupe_block_id_to_function_name', $ids);
...
function open_msupe_block_id_to_function_name ($id) {
